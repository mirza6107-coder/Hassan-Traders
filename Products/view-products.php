...
if (!$p) { mysqli_close($conn); header("Location: Products.php"); exit(); }
...
function parseDesc(string $raw): array {
    ...
    $flush = function() use (&$buffer, &$current, &$out) {
        ...
        foreach ($buffer as $l) {
            $t = trim($l);
            if (!$t) { if ($acc) $items[] = trim($acc); $acc = ''; continue; }
    ...
    foreach ($lines as $line) {
        $t = trim($line);
        if      (preg_match('/^key\s*features?\s*:/i',   $t)) { $flush(); $current='features'; }
        elseif  (preg_match('/^technical\s*spec/i',       $t)) { $flush(); $current='specs'; }
        elseif  (preg_match('/^ideal\s*for\s*:/i',        $t)) { $flush(); $current='ideal'; }
        elseif  (preg_match('/^(pro.?tip|installation\s*pro)/i', $t)) { $flush(); $current='protip'; }
        elseif  (preg_match('/^[A-Z][^.!?]{3,60}:$/', $t) && strlen($t)<70) { $flush(); $current=rtrim($t,':'); }
        else    { $buffer[] = $line; }
    }
    $flush();
    return $out;
...
<head>
  ...
  <style>
  :root {
    --primary:#C8102E; --primary-dark:#9E0B22;
    --primary-light:rgba(200,16,46,0.09);
    --cream:#FAF8F5; --border:#E8E2D9;
  ...
  /* BREADCRUMB */
  .vp-crumb { padding:18px 0 0; font-size:12.5px; color:var(--text-muted); font-weight:500; }
  .vp-crumb a { color:var(--text-muted); text-decoration:none; transition:color .18s; }
  .vp-crumb a:hover { color:var(--primary); }
  .vp-crumb .sep { margin:0 7px; opacity:.4; }
  .vp-crumb .cur { color:var(--text); font-weight:700; }

  /* MAIN SECTION */
  .vp-main { padding:28px 0 60px; }

  /* ── GALLERY ── */
  .vp-gallery { position:sticky; top:88px; }

  .vp-main-img {
    border-radius:var(--radius); overflow:hidden;
    background:#F2EDE8; aspect-ratio:1/1;
  ...
  .vp-main-img:hover img { transform:scale(1.04); }

  .vp-badge-cat {
    position:absolute; top:14px; left:14px;
    background:var(--primary); color:#fff;
    font-size:10px; font-weight:800; letter-spacing:.1em; text-transform:uppercase;
    padding:5px 13px; border-radius:50px; z-index:2;
  }
  .vp-badge-disc {
  ...
  /* Thumbnails */
  .vp-thumbs { display:flex; gap:10px; margin-top:14px; flex-wrap:wrap; }
  .vp-thumb {
    width:76px; height:76px; border-radius:10px; overflow:hidden;
    border:2px solid transparent; cursor:pointer;
    background:#F2EDE8; flex-shrink:0;
    transition:border-color .18s, transform .18s;
    box-shadow:0 2px 8px rgba(0,0,0,.06);
  }
  .vp-thumb img { width:100%; height:100%; object-fit:cover; display:block; }
  .vp-thumb.active { border-color:var(--primary); }
  .vp-thumb:hover { transform:scale(1.06); border-color:var(--primary); }
  ...
  /* ── INFO PANEL ── */
  .vp-info { padding-left:10px; }

  .vp-pills { display:flex; gap:8px; margin-bottom:14px; flex-wrap:wrap; }
  .vp-pill-cat {
  ...
  .vp-pill-brand {
    background:#F2EDE8; color:var(--text-muted);
  ...
  .vp-title {
    ...
    font-weight:900; line-height:1.2; color:var(--text); margin-bottom:18px;
  }

  .vp-price-row { display:flex; align-items:baseline; gap:13px; margin-bottom:18px; flex-wrap:wrap; }
  .vp-price { font-family:var(--serif); font-size:2.2rem; font-weight:700; color:var(--text); }
  .vp-orig  { font-size:15px; color:var(--text-muted); text-decoration:line-through; font-family:var(--mono); }
  .vp-disc-tag {
    background:var(--primary); color:#fff;
    font-size:11px; font-weight:800; padding:4px 11px; border-radius:50px;
  }

  .vp-stock {
    display:inline-flex; align-items:center; gap:7px;
    padding:7px 16px; border-radius:50px; font-size:12.5px; font-weight:700;
    margin-bottom:22px;
  }
  .vp-stock.ok  { background:#f0fdf4; color:#16a34a; border:1px solid #bbf7d0; }
  .vp-stock.low { background:#fffbeb; color:#d97706; border:1px solid #fde68a; }
  .vp-stock.out { background:#fef2f2; color:var(--primary); border:1px solid #fecaca; }

  .vp-hr { border:none; border-top:1px solid var(--border); margin:20px 0; }

  .vp-intro { font-size:14px; color:#4a4a4a; line-height:1.82; margin-bottom:22px; }

  /* Quick spec chips */
  .vp-chips { display:grid; grid-template-columns:1fr 1fr; gap:10px; margin-bottom:4px; }
  .vp-chip { background:var(--white); border:1px solid var(--border); border-radius:10px; padding:10px 14px; }
  .vp-chip-lbl { font-size:10px; font-weight:700; text-transform:uppercase; letter-spacing:.08em; color:var(--text-muted); margin-bottom:3px; }
  .vp-chip-val { font-size:13.5px; font-weight:700; color:var(--text); font-family:var(--mono); }

  /* CTA */
  .vp-cta { display:flex; gap:11px; margin-bottom:26px; flex-wrap:wrap; }
  .vp-btn-cart {
  ...
  .vp-btn-cart:hover { background:var(--primary-dark); transform:translateY(-2px); box-shadow:0 12px 32px rgba(200,16,46,.3); }
  .vp-btn-cart:disabled { opacity:.5; cursor:not-allowed; transform:none; }
  ...
  .vp-btn-wa:hover { background:#1da851; color:#fff; transform:translateY(-2px); }
  .vp-btn-back {
    display:inline-flex; align-items:center; gap:7px;
    font-size:13px; color:var(--text-muted); font-weight:600;
    text-decoration:none; transition:color .18s;
  }
  .vp-btn-back:hover { color:var(--primary); }

  /* Trust row */
  .vp-trust { display:flex; gap:20px; flex-wrap:wrap; font-size:12.5px; color:var(--text-muted); font-weight:600; }
  .vp-trust span { display:flex; align-items:center; gap:5px; }
  .vp-trust i { color:var(--primary); }

  /* ── TABS ── */
  .vp-tabs-wrap {
    background:var(--white); border-radius:var(--radius);
    border:1.5px solid var(--border); overflow:hidden;
    box-shadow:var(--shadow); margin:40px 0 0;
  }
  .vp-tab-nav {
  ...
  .vp-tab-btn {
    ...
    margin-bottom:-1.5px; transition:color .18s, border-color .18s;
  }
  .vp-tab-btn:hover { color:var(--text); }
  .vp-tab-btn.active { color:var(--primary); border-bottom-color:var(--primary); }
  .vp-tab-panel { display:none; padding:32px; }
  ...
  .vp-sec-head i { color:var(--primary); font-size:13px; }
  .vp-sec-head::after { content:''; flex:1; height:1px; background:var(--border); }

  .vp-feat-ul { list-style:none; padding:0; margin:0 0 28px; display:flex; flex-direction:column; gap:12px; }
  .vp-feat-ul li { display:flex; gap:12px; align-items:flex-start; font-size:14px; color:#3a3a3a; line-height:1.72; }
  .vp-feat-dot {
  ...
  .vp-feat-lbl { font-weight:700; color:var(--text); }

  .vp-ideal-ul { list-style:none; padding:0; margin:0 0 26px; display:flex; flex-direction:column; gap:10px; }
  .vp-ideal-ul li { display:flex; gap:8px; align-items:flex-start; font-size:14px; color:#3a3a3a; line-height:1.65; }
  .vp-ideal-arr { color:var(--primary); font-size:20px; line-height:1; margin-top:1px; flex-shrink:0; }
  .vp-ideal-lbl { font-weight:700; color:var(--text); }

  .vp-protip {
    background:#fffbeb; border:1px solid #fde68a;
    border-left:4px solid #f59e0b; border-radius:12px;
    padding:16px 20px; margin-top:8px;
    font-size:13.5px; color:#78350f; line-height:1.72;
  }
  .vp-protip p { margin:0; }
  .vp-protip-head {
    font-weight:800; font-size:11px; letter-spacing:.08em;
    text-transform:uppercase; color:#92400e; margin-bottom:7px;
    display:flex; align-items:center; gap:6px;
  }
  .vp-protip-head i { color:#f59e0b; }
  ...
  .vp-spec-tbl tbody tr { border-bottom:1px solid var(--border); }
  .vp-spec-tbl tbody tr:last-child { border-bottom:none; }
  .vp-spec-tbl tbody tr:nth-child(even) { background:var(--cream); }
  .vp-spec-tbl td { padding:11px 18px; font-size:13.5px; vertical-align:middle; }
  .vp-spec-tbl .sk { font-weight:700; color:var(--text); width:40%; }
  .vp-spec-tbl .sv { color:#4a4a4a; font-family:var(--mono); font-size:13px; }

  /* ── RELATED ── */
  .vp-related { padding:52px 0 72px; }
  .vp-rel-title { font-family:var(--serif); font-size:1.85rem; font-weight:700; color:var(--text); margin-bottom:28px; }
  .vp-rel-title span { color:var(--primary); }
  ...
  .vp-rel-card:hover { transform:translateY(-5px); box-shadow:0 18px 48px rgba(0,0,0,.10); border-color:rgba(200,16,46,.2); color:inherit; }
  .vp-rel-img { height:175px; background:#F2EDE8; overflow:hidden; }
  .vp-rel-img img { width:100%; height:100%; object-fit:cover; transition:transform .5s; display:block; }
  .vp-rel-card:hover .vp-rel-img img { transform:scale(1.06); }
  .vp-rel-body { padding:14px 16px 18px; }
  .vp-rel-brand { font-size:10px; font-weight:700; letter-spacing:.12em; text-transform:uppercase; color:var(--primary); margin-bottom:5px; }
  .vp-rel-name { font-size:14px; font-weight:700; color:var(--text); line-height:1.4; margin-bottom:10px; }
  .vp-rel-price { font-family:var(--serif); font-size:1.1rem; font-weight:700; color:var(--text); }
  .vp-rel-price span { font-size:12px; font-family:var(--font); color:var(--text-muted); font-weight:400; }

  /* ANIMATIONS */
  @keyframes fadeUp { from{opacity:0;transform:translateY(18px)} to{opacity:1;transform:translateY(0)} }
  .vp-gallery  { animation:fadeUp .42s .04s ease both; }
  .vp-info     { animation:fadeUp .42s .12s ease both; }
  .vp-tabs-wrap{ animation:fadeUp .42s .22s ease both; }
  .vp-rel-card:nth-child(1){animation:fadeUp .38s .06s ease both}
  .vp-rel-card:nth-child(2){animation:fadeUp .38s .12s ease both}
  .vp-rel-card:nth-child(3){animation:fadeUp .38s .18s ease both}
  .vp-rel-card:nth-child(4){animation:fadeUp .38s .24s ease both}

  @media(max-width:768px){
    .vp-gallery{position:static;margin-bottom:28px;}
    .vp-info{padding-left:0;}
    .vp-chips{grid-template-columns:1fr;}
    .vp-tab-panel{padding:18px 14px;}
  }
  </style>
</head>
<body>
      ...
      <div class="d-flex mt-3 mt-lg-0">
        <?php if(isset($_SESSION['user_name'])): ?>
          <div class="dropdown">
            ...
            <ul class="dropdown-menu"><li><a class="dropdown-item" href="../login and signup/logout.php">Logout</a></li></ul>
          </div>
        <?php else: ?>
  ...
  <div class="vp-tabs-wrap">
    ...
    <div class="vp-tab-panel active" id="tab-overview">
      ...
      <table class="vp-spec-tbl">
        ...
        <tbody>
          <?php if($p['Brand']): ?>
            <tr><td class="sk">Brand</td><td class="sv"><?= htmlspecialchars($p['Brand']) ?></td></tr>
          <?php endif; ?>
          <?php if($p['Category']): ?>
            <tr><td class="sk">Category</td><td class="sv"><?= htmlspecialchars($p['Category']) ?></td></tr>
          <?php endif; ?>
          <tr><td class="sk">Selling Price</td><td class="sv">Rs. <?= number_format($p['Price']) ?></td></tr>
          <?php if($hasOrig): ?>
            <tr><td class="sk">Original Price</td><td class="sv">Rs. <?= number_format($p['orignalprice']) ?></td></tr>
            <tr><td class="sk">Discount</td><td class="sv"><?= $discount ?>% Off</td></tr>
          <?php endif; ?>
          <tr><td class="sk">Availability</td><td class="sv"><?= $sLabel ?></td></tr>
          <?php if($p['STATUS']): ?>
            <tr><td class="sk">Status</td><td class="sv"><?= ucfirst($p['STATUS']) ?></td></tr>
          <?php endif; ?>
        </tbody>
      </table>
...
<script>
  ...
  function switchImg(src, el) {
    ...
    document.querySelectorAll(".vp-thumb").forEach(t => { t.classList.remove("active"); t.style.opacity = ".35"; });
