...
// Pull saved shipping info from session (set by place_order.php after first order)
$saved = [
    'name'    => $_SESSION['checkout_name']    ?? '',
    'phone'   => $_SESSION['checkout_phone']   ?? '',
    'email'   => $_SESSION['checkout_email']   ?? '',
    'address' => $_SESSION['checkout_address'] ?? '',
    'city'    => $_SESSION['checkout_city']    ?? 'Sargodha',
];
$hasSaved = !empty($saved['name']) && !empty($saved['phone']);
  ...
  <head>
    ...
    <style>
      ...
      .af-banner {
        ...
        gap: 11px;
        margin-bottom: 20px;
        animation: afSlide .35s ease;
      }
      @keyframes afSlide {
        from { opacity:0; transform:translateY(-8px); }
        to   { opacity:1; transform:translateY(0); }
      }
      .af-banner i   { color:#059669; font-size:1.2rem; flex-shrink:0; margin-top:2px; }
      .af-banner strong { color:#065f46; font-size:.88rem; display:block; }
      .af-banner span   { color:#047857; font-size:.8rem; }
      ...
      /* ── Navbar dropdown (matches rest of site) ── */
      .ht-dd-menu {
            ...
            <?php if ($userName): ?>
              ...
              <div class="dropdown">
                <button class="btn btn-premium px-4 rounded-pill dropdown-toggle d-flex align-items-center gap-2"
                  ...
                  <span class="fw-semibold" style="max-width:120px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">
                ...
                <ul class="dropdown-menu dropdown-menu-end ht-dd-menu" aria-labelledby="userDropdownBtn">
                  ...
                  <li><a class="dropdown-item" href="../Profile/profile.php"><i class="bi bi-person-badge-fill"></i> My Profile</a></li>
                  <li><a class="dropdown-item" href="../Profile/profile.php#orders"><i class="bi bi-bag-heart-fill"></i> My Orders</a></li>
                  <li><a class="dropdown-item" href="../Add to Cart and CheckOut/Cart.php"><i class="bi bi-cart3"></i> My Cart</a></li>
                  <li><hr class="dropdown-divider"/></li>
                  <li><a class="dropdown-item logout-item" href="../login and signup/logout.php"><i class="bi bi-box-arrow-right"></i> Logout</a></li>
                </ul>
              </div>
            <?php else: ?>
      ...
      <div class="container">
        <div class="row g-4 align-items-start">
          ...
          <div class="col-lg-7">
            ...
            <!-- Auto-fill banner (only shown when previous order exists) -->
            <?php if ($hasSaved): ?>
            <div class="af-banner">
              <i class="bi bi-magic"></i>
              <div>
                <strong>Details auto-filled from your last order!</strong>
                <span>Review and update anything that has changed.</span>
              </div>
            </div>
            <?php endif; ?>
                ...
                <div class="row g-3">
                  <div class="col-md-6">
                    ...
                    <input type="text" id="userName"
                           class="co-input <?= $hasSaved && $saved['name'] ? 'autofilled' : '' ?>"
                           value="<?= htmlspecialchars($saved['name']) ?>"
                           placeholder="e.g. Muhammad Hassan" required />
                  </div>
                  <div class="col-md-6">
                    <label class="co-label">Phone Number <span class="co-req">*</span></label>
                    <input type="tel" id="userPhone"
                           class="co-input <?= $hasSaved && $saved['phone'] ? 'autofilled' : '' ?>"
                  ...
                  <div class="col-md-6">
                    ...
                    <input type="email" id="userEmail"
                           class="co-input <?= $hasSaved && $saved['email'] ? 'autofilled' : '' ?>"
                           value="<?= htmlspecialchars($saved['email']) ?>"
                ...
                <div class="row g-3">
                  <div class="col-12">
                    ...
                    <input type="text" id="userAddress"
                           class="co-input <?= $hasSaved && $saved['address'] ? 'autofilled' : '' ?>"
                           value="<?= htmlspecialchars($saved['address']) ?>"
                           placeholder="House/Plot No., Street Name, Area" required />
                  </div>
                  <div class="col-md-6">
                    <label class="co-label">City <span class="co-req">*</span></label>
                    <select id="userCity" class="co-input co-select">
                      <?php
                      $cities = ['Sargodha','Lahore','Faisalabad','Rawalpindi','Other'];
                      foreach ($cities as $c) {
                          $sel = ($saved['city'] === $c) ? 'selected' : '';
    ...
    <div class="co-success-overlay" id="successOverlay" style="display:none;">
      <div class="co-success-modal">
        ...
        <div style="display:flex;gap:12px;justify-content:center;flex-wrap:wrap;">
          <a href="../Profile/profile.php#orders" class="co-success-btn">
            <i class="bi bi-bag-heart me-1"></i> My Orders
          </a>
          <a href="../Home/home.php" class="co-success-btn" style="background:#1a1a2e;">
